    ...
    for (int x=x_min;x<=x_max;x++)
    {
        for (int y=y_min;y<=y_max;y++)
        {
            ...
            if (insideTriangle(x+0.5,y+0.5,t.v))
            {
                ...
                float w_reciprocal = 1.0/(alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w()); //w value of homogeneous coordinate
                float z_interpolated = alpha * v[0].z() / v[0].w() + beta * v[1].z() / v[1].w() + gamma * v[2].z() / v[2].w();//calculate the value of Z in homogeneous cor
